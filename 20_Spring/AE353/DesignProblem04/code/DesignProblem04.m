...
process.bodyHeight = 1.8*(process.l+process.bodyDepth);
% - Masses (kg)
...
if (exist(process.roadfile,'file')==2)
    ...
    process.road = road;
    % FIXME: should standardize and not load this, because we may end up
    % with a different roadwidth in 'DP04_eom.mat' as in simulation
...
if ~isempty(process.eomfile)
    if ~strcmp(process.eomfile, 'DP04_eom.mat')
        warning(['During the race, the equations of motion ', ...
                 'will be available in the file ''DP04_eom.mat'' ', ...
                 sprintf('and NOT in the file ''%s'' that you ', process.eomfile), ...
                 'are creating.']);
    end
    [symEOM,numEOM] = GetEOM(process.g,      ...
                             ...
                             process.r,      ...
                             process.l);
    % FIXME: should not hard-code this (duplicated later for controller)
	parameters = struct;
    parameters.tStep = process.tStep;
    parameters.tauMax = process.tauMax;
    parameters.roadwidth = process.roadwidth;
    parameters.b = process.b;
    parameters.r = process.r;
    save(process.eomfile, 'symEOM', 'numEOM', 'parameters');
end
process.numEOM_f = @(phi, phidot, v, w, tauR, tauL) ...
    EvaluateEOM(phi, phidot, v, w, tauR, tauL, ...
                process.g,      ...
                process.Jx,     ...
                process.Jy,     ...
                process.Jz,     ...
...
process.result = nan;
[process.dR, process.dL] = GetDistances(process);
...
names = {'tStep','tauMax','roadwidth','b','r'};
...
function res = EvaluateEOM(phi,phidot,v,w,tauR,tauL, ...   % states and inputs
                           g,Jx,Jy,Jz,Jw,mo,mw,b,r,l)      % parameters
J5 = (Jx+mo*l^2)*sin(phi)^2+Jz*cos(phi)^2+(3/4)*mw*b^2+(1/2)*mw*r^2;
M = [Jy+mo*l^2              mo*l*cos(phi)             0;
     mo*l*cos(phi)        mo+2*mw+(2*Jw/r^2)          0;
     0                      0                           J5];
...
sensors.dR = process.dR;
sensors.dL = process.dL;
...
if all(isfield(actuators,{'tauR','tauL'}))&&(length(fieldnames(actuators))==2)
    if isnumeric(actuators.tauR)&&isnumeric(actuators.tauL)
        if isscalar(actuators.tauR)&&isscalar(actuators.tauL)
            if (~isnan(actuators.tauR))&&(~isinf(actuators.tauR))&& ...
               (~isnan(actuators.tauL))&&(~isinf(actuators.tauL))&& ...
               (isreal(actuators.tauR))&&(isreal(actuators.tauL))
                iscorrect = true;
            end
        end
    end
...
if (isempty(fig))
    ...
    % - laser scans
    fig.view0.dR = line(0, 0, 'color', 'r', 'linewidth', 3, 'linestyle', '-');
    fig.view0.dL = line(0, 0, 'color', 'r', 'linewidth', 3, 'linestyle', ':');
...
set(fig.view0.dR, 'xdata', process.x + (0.5*process.b + [0, process.dR]) * sin(process.theta), ...
                  'ydata', process.y - (0.5*process.b + [0, process.dR]) * cos(process.theta));
set(fig.view0.dL, 'xdata', process.x - (0.5*process.b + [0, process.dL]) * sin(process.theta), ...
                  'ydata', process.y + (0.5*process.b + [0, process.dL]) * cos(process.theta));
...
while (1)
    ...
    if ((process.t + (process.tStep / 2) >= process.tStop)||done||ShouldStop(process))
        break;
    end
...
function [dR, dL] = GetDistances(process)
% Both searches complete in fewer than 20 iterations with current
% tolerances, so we will simply cap the number of iterations at 20.
%
% We could certainly reduce this number, and accept a little more noise in
% the measurements.
maxNumIters = 20;
...
w = process.road.roadwidth / 2;
% Get distance off right wheel
smin = max(process.sClosest - 5, process.road.s(1));
smax = min(process.sClosest + 5, process.road.s(end));
for i = 1:maxNumIters
    s = 0.5 * (smin + smax);
    q = WhereAmI(s, 1, process.road);
    xQ = q(1) + w * sin(q(3));
    yQ = q(2) - w * cos(q(3));
    d = n' * [xQ - x; yQ - y];
    if (smax - smin <= 1e-4)
...
dL = norm([xQ-x, yQ-y]);
% Subtract half of the wheel base
dR = dR - 0.5 * process.b;
dL = dL - 0.5 * process.b;
end
...
for i=1:10:size(q,2)
    ...
    theta = q(3,i)+pi/2;
    r = road.roadwidth/2;
    h.hash(:,end+1) = plot([x-r*cos(theta) x+r*cos(theta)],...
                           [y-r*sin(theta) y+r*sin(theta)],...
                            '-','linewidth',1,'color',0.85*[1 1 1]);
end
end
...
if isempty(robotfig)
    % - links
    robotfig.chassis = patch('Vertices',robot.chassis.p_in0',...
                             'Faces',robot.chassis.faces,...
                             'FaceVertexCData',robot.chassis.colors,...
                             'FaceColor','flat',...
                             'FaceAlpha',alpha,'EdgeAlpha',alpha,...
                             'backfacelighting','reverselit',...
                             'AmbientStrength',0.6);
	robotfig.wheel2 = patch('Vertices',robot.wheel2.p_in0',...
                            'Faces',robot.wheel2.faces,...
                            'FaceVertexCData',robot.wheel2.colors,...
                            'FaceColor','flat',...
                            'FaceAlpha',alpha,...
                            'EdgeAlpha',alpha,...
                            'backfacelighting','reverselit',...
                            'AmbientStrength',0.6,...
                            'linewidth',0.01);
	robotfig.wheel3 = patch('Vertices',robot.wheel3.p_in0',...
                            'Faces',robot.wheel3.faces,...
                            'FaceVertexCData',robot.wheel3.colors,...
                            'FaceColor','flat',...
                            'FaceAlpha',alpha,...
                            'EdgeAlpha',alpha,...
                            'backfacelighting','reverselit',...
