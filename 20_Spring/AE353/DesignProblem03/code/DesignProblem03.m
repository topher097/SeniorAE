...
%
%           'reference' : a function of time that specifies a desired pitch
%                         angle - for example, the following choice would
...
process.numEOM_f = @(theta, phi, xdot, ydot, thetadot, phidot) ...
    EvaluateEOM(theta, phi, xdot, ydot, thetadot, phidot, ...
                process.m,      ...
                process.g,      ...
                process.J,      ...
                process.rho,    ...
...
function res = EvaluateEOM(theta, phi, xdot, ydot, thetadot, phidot, ...   % states
                           m, g, J, rho, Sw, Se, l, lw, le)                % parameters
xwdot = [xdot+lw*thetadot*sin(theta);
         ydot-lw*thetadot*cos(theta)];
xedot = [xdot+l*thetadot*sin(theta)+le*(thetadot+phidot)*sin(theta+phi);
         ydot-l*thetadot*cos(theta)-le*(thetadot+phidot)*cos(theta+phi)];
...
sensors.airspeed = process.xdot * cos(process.theta) + process.ydot * sin(process.theta);
if sensors.airspeed < 0
    sensors.airspeed = 0;
end
% Add noise
%   (nothing)
...
thetadot = x(7,1);
phidot = u(1,1);
xdot = [xdot; ydot; thetadot; phidot; process.numEOM_f(theta,phi,xdot,ydot,thetadot,phidot)];
end
...
if all(isfield(actuators,{'phidot'}))&&(length(fieldnames(actuators))==1)
    if isnumeric(actuators.phidot)
        if isscalar(actuators.phidot)
            if (~isnan(actuators.phidot))&&(~isinf(actuators.phidot))
                iscorrect = true;
            end
        end
    end
