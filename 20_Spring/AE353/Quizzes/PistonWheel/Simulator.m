...
% Simulator   run simulation of flywheel with piston
%
...
%                           @(t) sin(t)
%
%           'initial' : a 2x1 matrix [alpha; alphadot] that specifies
...
process.processdatatolog = {'t','alpha','alphadot'};
...
m = 10;
b = 0.4;
process.f = @(alpha, alphadot, tau) ...
    [alphadot;
     -(2*m*sin(2*alpha)*alphadot^2*r^2 + b*alphadot - tau)/(J + 4*m*r^2*sin(alpha)^2)];
process.r = r;
process.tauMax = 3;
...
process.t = 0;
% Initial conditions
process.alpha = process.initial(1,1);
process.alphadot = process.initial(2,1);
...
sensors.t = process.t;
sensors.d = 2 * process.r * cos(process.alpha);
end
...
t = process.t;
x = [process.alpha; process.alphadot];
end
...
tau = u(1,1);
% compute xdot
xdot = process.f(alpha, alphadot, tau);
end
...
if (isempty(fig))
    ...
    if process.diagnostics
        ...
        fig.x.legend = legend({'alpha','alphadot'});
        
        fig.y.axis = axes('position',[0.55,0.4,0.4,0.225],'fontsize',fs);
        axis([0,process.tStop,0,3*process.r]);
        hold on;
        grid on;
    ...
    fig.piston = line([r*cos(q), x], [r*sin(q), y], 'color', 'k', 'linewidth', 4);
    fig.p = rectangle('position', [r * cos(q) - w, r * sin(q) - w, 2*w, 2*w], 'curvature', [1 1], 'facecolor', 'w', 'edgecolor', 'k');
    fig.shaft = rectangle('position', [-10, -w, 20, 2*w], 'curvature', [0 0], 'facecolor', 0.9 * [1 1 1], 'edgecolor', 'k', 'linewidth', 1);
    fig.center = rectangle('position', [-w, -w, 2*w, 2*w], 'curvature', [1 1], 'facecolor', 'w', 'edgecolor', 'k');
    fig.mass = rectangle('position', [x - 2*w, y - 2*w, 4*w, 4*w], 'curvature', [0 0], 'facecolor', 'y', 'edgecolor', 'k');
...
if process.diagnostics
    t = get(fig.x.q,'xdata');
    x = get(fig.x.q,'ydata');
    set(fig.x.q,'xdata',[t process.t],'ydata',[x, process.alpha]);
    
    t = get(fig.x.qd,'xdata');
    x = get(fig.x.qd,'ydata');
    set(fig.x.qd,'xdata',[t process.t],'ydata',[x, process.alphadot]);
...
set(fig.q, 'position', [x - w, y - w, 2*w, 2*w]);
set(fig.piston, 'xdata', [r*cos(q), x], 'ydata', [r*sin(q), y]);
set(fig.p_text, 'position', [(1+s)*r*cos(q), (1+s)*r*sin(q), 0]);
set(fig.q_text, 'position', [x+s*r, y+s*r, 0]);
set(fig.mass, 'position', [x - 2*w, y - 2*w, 4*w, 4*w]);
...
while (1)
    ...
    if (abs(process.alpha) >= (pi / 2))
        warning(['The angle of the flywheel is:\n  %.3f\n\n' ...
             'This is outside the bounds:\n  [-pi/2, pi/2]\n\n' ...
