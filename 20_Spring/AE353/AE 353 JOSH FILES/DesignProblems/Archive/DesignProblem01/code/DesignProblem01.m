...
sensors.t = process.t;
sensors.w1 = process.w1;
sensors.w2 = process.w2;
sensors.w3 = process.w3;
% Add noise
%   (nothing)
end
...
u = [actuators.tau1; actuators.tau3];
...
iscorrect = false;
if all(isfield(actuators,{'tau1','tau3'}))&&(length(fieldnames(actuators))==2)
    if isnumeric(actuators.tau1) && isnumeric(actuators.tau3)
        if isscalar(actuators.tau1) && isscalar(actuators.tau3)
            iscorrect = true;
        end
    end
end
...
actuators = struct('tau1',0,'tau3',0);
end
...
if (isempty(fig))
    ...
    if process.diagnostics
        ...
        fig.u.tau3 = plot(nan,nan,'linewidth',3);
...
if process.diagnostics
    ...
    tau3 = [get(fig.u.tau3,'ydata') controller.actuators.tau3];
    set(fig.u.tau1,'xdata',t,'ydata',tau1);
    set(fig.u.tau3,'xdata',t,'ydata',tau3);
end
