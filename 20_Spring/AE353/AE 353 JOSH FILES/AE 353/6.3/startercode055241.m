...
%%%%%%%%%%%%%%%%%%
% MODIFY
data.A = [0 1; -5*cos(-0.5) -8/16];
data.B = [0;1/16];
data.C = [1 0];
data.xhat = [0;0];
data.u=0;
data.K = acker(data.A,data.B,[-3,-1.5]);
data.L = acker(data.A',data.C',[-12,-6])';
data.K
data.L
%
%%%%%%%%%%%%%%%%%%
...
xdot = [x(2); (1/16)*(-(8*x(2)+80*sin(x(1)))+u(1))];
end
