...
try
    [command_window_output,controller.actuators,controller.data] = ...
        evalc(['controller.init(controller.sensors, ' ...
...
while (1)
    ...
    % Update data.
    for i=1:length(processList)
        if (~isempty(processList{i}.datafile) && controllerList{i}.running)
...
if (controller.running)
    ...
    if controller.tComputation > process.maxComputationTime
