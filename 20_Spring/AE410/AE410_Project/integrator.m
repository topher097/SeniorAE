function [D,t,u] = integrator(L, Nx, PeriodicFlag, T, Nt, u0, spacedef, casedef, eps)
% Inputs:
% 
...
% eps: artificial dissipation factor (see Ex7)
...
if strcmpi('Central2', spacedef)
    D1 = sparse(Central2(Nx, dx, PeriodicFlag));
elseif strcmpi('Central2Dissipation', spacedef)
    [D1, D2] = sparse(Central2Dissipation(Nx, dx, PeriodicFlag));
elseif strcmpi('UpwindBiased3', spacedef)
    D1 = sparse(UpwindBiased3(Nx, dx, PeriodicFlag));
elseif strcmpi('DownwindBiased3', spacedef)
    D1 = sparse(DownwindBiased3(Nx, dx, PeriodicFlag));
elseif strcmpi('Pade346', spacedef)
    D1 = sparse(Pade346(Nx, dx, PeriodicFlag));
...
elseif strcmpi('EulerODE', casedef)
    [t,u] = ode45(@(t,y) EulerODE(t,y,D1), t, u0);
elseif strcmpi('EulerODEDissipation', casedef)
    [t,u] = ode45(@(t,y) EulerODEDissipation(t,y,D1,D2,eps), t, u0);
elseif strcmpi('EulerODEChar', casedef)
    [t,u] = ode45(@(t,y) EulerODEChar(t,y,D1,D2,eps), t, u0);
else
...
%% Euler ODE with Artificial Dissipation
function qdot = EulerODEDissipation(t,q,D1,D2,eps)
    % properties
    g = 1.4;
...
%% Euler ODE with Characteristic Boundary Conditions
...
%% Central with Forward and 
function [C1,C2] = Central2Dissipation(Nx,dx,PeriodicFlag)
    % Rturns the two matrices for Euler ODE with Dissipation
