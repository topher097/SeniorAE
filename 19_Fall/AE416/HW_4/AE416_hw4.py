...
class homeworkFour():
    def __init__(self):
        self.cp_min_i = -0.6
        self.gamma = 1.4
        self.m_crit_plot = np.linspace(0.22, .99, 3000)
    ...
    def sectionA(self):
        ...
        cm.text(0.775, 0.83, info_text, transform=cm.transAxes, fontsize=11, verticalalignment='top', bbox=props)
        cm.legend(loc='upper right', fontsize=9)
    ...
    def sectionB(self):
        ...
        c_text = '$C_{p_{crit}}$'
        info_text = f'{m_text} = {round(m_crit, 5)}\n' \
                    f'{m2_text} = {round(m_crit/cos(np.deg2rad(25)), 5)}\n' \
                    f'{c_text} = {round(cp_crit, 5)}'
        print(f'M_crit_b (normal) = {round(m_crit, 5)}')
        print(f'M_crit_b (swept) = {round(m_crit/cos(np.deg2rad(25)), 5)}')
        ...
        plt.gca().invert_yaxis()
        cm.set_xlabel('$M_{crit} (normal)$', fontsize=16)
        cm.set_ylabel('$C_{p_{crit}}$', fontsize=16)
